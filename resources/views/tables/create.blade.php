        ...
        <div class="w-full max-w-md p-4 bg-white border border-gray-200 rounded-lg shadow sm:p-6 md:p-8 dark:bg-gray-800 dark:border-gray-700">
            ...
            @if($lastTable)
                <div class="mb-5 p-4 bg-green-100 text-green-800 rounded-lg">
                    <p><strong>Última Mesa Registrada:</strong> {{ $lastTable->name }}</p>
                </div>
            @endif

            <form class="max-w-lg mx-auto" action="{{route('tables.store')}}" method="POST" enctype="multipart/form-data">
                @csrf
               ...
               <div class="items-center gap-6">
                    <button type="submit"
                        class="mx-6 text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                        Registrar
                    </button>
